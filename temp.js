...
const animalNames = JSON.parse(fs.readFileSync('animalNames.json'));

const animalNameMap = {};
for (const animalName of animalNames) {
  animalNameMap[animalName] = true;
}
      ...
      fs.readFile("./data.json", (err, data) => {
        ...
        for (const imagePredictions of receivedData) {
            ...
            for (const prediction of imagePredictions) {
              ...
              if (animalNames.includes(animalName) || !animalNameMap[animalName]) {
                // Ignore animal names that have already been added or are not included in animalNames.json
                continue;
              }
